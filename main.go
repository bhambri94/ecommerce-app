...
import (
	...
	"github.com/bhambri94/ecommerce-app/overstock"
...
func main() {
	...
	router.GET("/v1/overstock/search", handleOverStockSearch)
...
func handleOverStockSearch(ctx *fasthttp.RequestCtx) {
	sugar.Infof("calling overstock search api with multiple products!")
	configs.SetConfig()
	loc, _ := time.LoadLocation("America/Bogota")
	currentTime := time.Now().In(loc)
	CSVName := "OverstockCSV" + currentTime.Format("2006-01-02 15:04:05") + ".csv"
	f, err := os.Create(CSVName)
	if err != nil {
		log.Fatal(err)
	}
	...
	header := []string{"Ebay_Refresh_time", "Product URL", "Product Title", "Price", "Quantity Available", "Items Feedback", "Notification", "Delivery Date", "Returns", "Image URLS", "Product Desciption Content"}
	writer.Write(header)
	productUrls, e := sheets.BatchGet("Overstock!A2:A55000")
	if e != nil {
		ctx.Response.SetStatusCode(200)
		ctx.Response.SetBody([]byte("URl format wrong in Sheets"))
		return
	...
	finalValues := overstock.GetPageDescription(Urls)
	stringfinalValues := make([][]string, len(finalValues)+5)
	i := 0
	for i < len(finalValues) {
		for _, value := range finalValues[i] {
